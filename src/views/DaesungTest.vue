<template>
  ...
  import { sendMessageToChatGPT } from '@/api/chatTest'
  ...
  const fetchAnswer = async () => {
    ...
    try {
      const answer = await sendMessageToChatGPT(question.value)
  
      if (answer) {
        const fullHtml = md.render(answer)
        answerHtml.value = fullHtml
  
        if (answer.length > 700) {
          isLongAnswer.value = true
          const shortText = answer.slice(0, 600) + "..."
